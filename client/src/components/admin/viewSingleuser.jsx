...
const UserView = () => {
  ...
  const handleBlockProduct = async (productId) => {
    try {
      const action = products.find((product) => product._id === productId)?.status === "active" ? "block" : "unblock";
      await axios.patch(`http://localhost:3005/products/${productId}/status`, { action });
      toast.success(`Product ${action}ed successfully!`);
      setProducts((prevProducts) =>
        prevProducts.map((product) =>
          product._id === productId ? { ...product, status: action === "block" ? "blocked" : "active" } : product
        )
      );
    } catch (err) {
      console.error("Error blocking/unblocking product:", err);
      toast.error(err.response?.data?.message || "Error blocking/unblocking product");
    }
  };
            ...
            >
              {products.map((product) => (
                ...
                >
                  ...
                  >
                    {product.status === "active" ? "Block Product" : "Unblock Product"}
                  </button>
                </div>
              ))}
            </div>
